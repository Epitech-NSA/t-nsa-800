...
function NumberInput(props: NumberInputProps): JSX.Element {
  ...
  function onValueChanged(event: ChangeEvent<HTMLInputElement>): void {
    let elementValue: number = isNaN(Number(event.target.value))
      ? 0
    ...
    if (!error) {
      [error, validClass] =
        props.max != null && elementValue > props.max
          ? [`Value can't be higher than ${props.max} `, "is-invalid"]
          : ["", "is-valid"];
    }

    if (!error) {
      [error, validClass] =
        props.min != null && elementValue < props.min
          ? [`Value can't be lower than ${props.min} `, "is-invalid"]
          : ["", "is-valid"];
    }
